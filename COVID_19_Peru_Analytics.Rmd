...
load.libraries <- c('plyr', 'dplyr','data.table', 'readxl', 'tibble',
                    'stringr', 'stringi','forecast', 'tidyverse', 'broom','gt',
                    'caret','zoo','plotly','DT','rpart','tidyr', 'rvest', 'hrbrthemes',
...
COVID_19_Peru_Raw %>%
  ...
  layout(title = "Percentage of Deaths for COVID-19 in Peru (Daily)",
     xaxis = list(title = "Date",
                  date_breaks="7 days", date_labels = "%d %b"),
                  #range = c(118,195)),
     yaxis = list(title = "Index of Deaths every 1 day",
                  tickformat = "%")
                  #range = c(10,30))
     )
...
## Using SIR model to analyze outbreak
...
It is quite clear that, if outbreak follows model above, it would be an unmitigated disaster. At this point, it is worth remarking on the importance of decisive public health intervention to limit the spread of such epidemics. Without such interventions, tens of millions of people could be infected, as our model predicts, and even with only a one or two per cent mortality rate, hundreds of thousands of people would die. 
