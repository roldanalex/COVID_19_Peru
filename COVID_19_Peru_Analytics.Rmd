...
load.libraries <- c('plyr', 'dplyr','data.table', 'readxl', 'tibble','deSolve',
...
p <- ggplot(ww, aes(days_since_100th_case, possitive_cased_confirmed, color = country)) +
  ...
  geom_shadowtext(aes(label = paste0(" ",country)), hjust=0, vjust = 0, 
                  data = . %>% group_by(country) %>% top_n(1, days_since_100th_case), 
                  bg.color = "white") +
  labs(x = "Number of days since 100th case", y = "", 
       title = "Daily possitive cases Worldwide")
...
z <- COVID_19_Peru_Raw_Inc2day %>%
  ggplot(aes(x = Date, 
             y = percent_cases_increase_2_days,
             text = paste('Date: ', Date,
                          '<br>Percentage (%):', round(percent_cases_increase_2_days*100,1)))) +
  # geom_line()  + geom_line() +
    geom_bar(stat="identity", fill = "darkblue") + 
    scale_x_date(date_breaks="7 days", date_labels = "%d %b") +
